    ...
    <Carousel plugins={[autoplayPlugin.current]} className="w-full max-w-lg mx-auto">
      <CarouselContent>
        {projects.map((project, index) => {
          const previewImage = project.previewImage as ProjectImage
          return (
                  ...
                  <Card className='rounded'>
                    {project.previewImage && (
                      <div className="absolute w-full h-full z-10 -ml-2 rounded">
                        <Image
                          ...
                          alt={previewImage.alt as string}
                          src={previewImage.sizes?.square_small?.url as string}
                        />
                      </div>
                    )}
                    <CardContent className="flex aspect-square items-center p-6 rounded">
                      ...
                      >
                        ...
                        <span
                          dangerouslySetInnerHTML={{ "__html": project.summary_html as TrustedHTML }}
